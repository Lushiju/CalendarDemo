package com.lushiju.calendardemo.view;
...
public class MonthDateView extends View {
	...
	private int mSelectBGColor = Color.parseColor("#FF4E00");
...
//				weekRow = row + 1;
//			}
//			if(dayString.equals(mSelDay+"")&&isSelect){
...
//			} else
	...
	private void drawCircle(int row,int column,int day,Canvas canvas){
		if(daysHasThingList != null && daysHasThingList.size() >0){
			...
			textPaint.setColor(mSelectDayColor);
			textPaint.setTextSize(12.5f*mDisplayMetrics.scaledDensity);
			StaticLayout layout = new StaticLayout(text,textPaint,300, Layout.Alignment.ALIGN_NORMAL, 0.0F, 0.0F, true);
			// 这里的参数300，表示字符串的长度，当满300时，就会换行，也可以使用“\r\n”来实现换行
			canvas.save();
			int startXt;
			if (text.equals("客满")){
				startXt = (int) (mColumnSize * column + (mColumnSize - mPaint.measureText(text)));
			}else {
				startXt = (int) (mColumnSize * column+text.length()*3);

			}
			canvas.translate(startXt,startY);//从100，100开始画
			layout.draw(canvas);
	...
	public String getSelectYearMonth(){
